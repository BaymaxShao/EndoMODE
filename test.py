...
def main():
    ...
    ATEs, RTEs, ROTs, CEs, traj_pred, traj_gt, d_pred, d_gt, rel_pred, rel_gt = test(test_loader, model)
    # ATEs, RTEs, ROTs, CEs, traj_pred, traj_gt, d_pred, d_gt, rel_pred, rel_gt = test_rel(test_loader, model)
...
# Test the absolute pose estimation
def test(test_loader, model):
    ...
    for i, (img1, img2, ref_pose, tar_pose, rel_pose) in enumerate(test_loader):
        ...
        print('ATE: {:.4f} mm, CE: {:.4f}'.format(ATEs[-1], CEs[-1]))
        print('Error of relative transition >>>')
        print('RTE: {:.4f} mm, ROT: {:.4f} deg'.format(RTEs[-1], ROTs[-1]))

    return np.array(ATEs), np.array(RTEs), np.array(ROTs), np.array(CEs), traj_pred, traj_gt, d_pred, d_gt, rel_poses_pred, rel_poses_gt


# Test the relative pose prediction
def test_rel(test_loader, model):
    traj_gt = []
    traj_pred = []
    Ts_gt = []
    Ts_pred = []
    ...
    for i, (img1, img2, ref_pose, tar_pose, rel_pose) in enumerate(test_loader):
        ...
        # Predict the relative transformation [t,r]
        # rel_pose_pred = model(img1, img2, flow).cpu()
        # rel_pose_pred = model(img1, img2, flow)
        rel_pose_pred = model(img1, img2)
...
def save_results(save_path, obj, ates, rtes, rots, ces, traj_pred, traj_gt, d_pred, d_gt, rel_pred, rel_gt):
