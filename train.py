...
def main():
    ...
    #     istrain=False,
    #     img_size=224,
    #     train_list='val_file.txt',
    #     test_list='test_file.txt',
    #     step=1
    # )

    print('=== {} samples found in {} train objects ==='.format(len(train_set), len(train_set.objs)))
    # print('=== {} samples found in {} valid objects ==='.format(len(val_set), len(val_set.objs)))
...
# def validate(val_loader, model, epoch, state):
#     model.eval()
#     losses = []
#     for i, (img1, img2, ref_pose, tar_pose, rel_pose_gt) in enumerate(tqdm(val_loader)):
#         img1 = img1.to(device)
#         img2 = img2.to(device)
#
#         rel_pose_gt = rel_pose_gt.to(device)
